...
| 指標 | 上限 |
|---|---|
| 軌跡產生(7200 幀場次) | 400 ms(桌機);行動裝置見下方修訂 |
| 播放時的顯示更新率 | 60 fps 穩定(桌機)/ 30 fps 以上(行動裝置) |
| 診斷啟用時的軌跡產生 | 500 ms |

**不得為效能調整物理參數或降低 solver 迭代次數。**

效能不足時可動用的手段**僅限渲染層**:降低陰影品質或關閉陰影、降低 pixel ratio、
簡化材質、降低圍欄與場地曲線的 mesh 細分。
降低 solver 迭代次數、改變 timestep、跳幀模擬、簡化碰撞體一律不在選項內 —— 見 §P1.11。

### 修訂:Web Worker(Phase 1-e,2026-08-18)

**原條文作廢:**

> **不實作 Web Worker。** 實測 309 ms 與診斷版 300 ms 的差異落在量測噪音內;
> 且該情況僅發生於 7200 幀場次,而該類場次正是極限環繞修正後將要消除的。
> 為一個即將消失的邊界情況引入非同步架構,不符本專案的複雜度取捨。

**作廢理由 —— 原否決的兩個前提在行動裝置上都不成立:**

1. **不是噪音等級的差異。** Chrome 行動視窗 + 4× CPU 節流實測,7200 幀場次的軌跡產生
   為 **1655–1721 ms**,是 400 ms 預算的 4 倍以上。同一段計算在主執行緒同步執行,
   期間畫面完全凍結、無法互動。
2. **不是即將消失的邊界情況。** Phase 1.5 第二輪實測,基線場地的逾時率為 16.8%,
   即約每六場就有一場會遇到近兩秒的無回應。極限環繞的修正尚未實施,
   且第一輪 μ 掃描顯示降低 μ 會**惡化**環繞,該修正的難度高於原先預期。

**新條文:軌跡產生於瀏覽器端移入 Web Worker。**

實作約束:

1. **僅將軌跡產生移入 Worker。軌跡仍完整產生後才開始播放。**
   §P1.2.1 的結構保證不得改變 —— Worker 內跑的是完整的 `generateTrajectory()`,
   一次跑完回傳一份定案的軌跡,**不是逐幀串流給渲染層**。
   「產生」與「播放」的分界沒有移動,只是第一段換了一條執行緒。
2. **產生期間 UI 保持可互動**,並顯示進度指示。
   進度為**不確定進度**(僅顯示經過時間):`simulate()` 不提供幀數進度,
   要取得它必須在物理主迴圈內插入回呼,屬 `src/sim/` 的變更,不在授權範圍內。
3. **Worker 與主執行緒的結果必須位元相同**,以每一個型別化陣列逐位元比對驗證,
   並納入測試(`tests/worker.test.ts`,以 `node:worker_threads` 起真執行緒)。
4. **`verify-platform --compare` 仍須 20/20。**

檔案配置(受 §P1.6 的分層約束):

| 檔案 | 位置 | 理由 |
|---|---|---|
| `trajectory-job.ts` | `src/replay/` | 協定與執行本體,環境中立,node 可直接測試 |
| `trajectory-worker.ts` | `src/render/` | 使用 `self`,是瀏覽器 API |
| `trajectory-client.ts` | `src/render/` | 使用 `Worker` / `performance`,是瀏覽器 API |

後兩者**不得**放在 `src/replay/` —— §P1.6 規定該層同時服務 CLI 與渲染層,
不得使用瀏覽器 API,ESLint 規則會擋下。CLI 不需要把計算移出主執行緒。

沒有 `Worker` 的環境(node 測試、`tools/`)退回當前執行緒執行,結果相同。
