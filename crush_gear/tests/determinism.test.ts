...
type Scenario = {
   ...
   * 覆寫重複次數。
   *
   * ⚠️ **已知偏離 §11.1**：規格要求每組參數連續執行 100 次。唯一被降低的是
   * 「逾時平手」情境（`repeats: 20`），因為它每次都要跑滿 7200 幀。
   * 成本：單次約 320 ms，20 次約 6.4 秒，100 次約 32 秒 ——
   * 會讓整個測試套件由約 140 秒增加到約 165 秒，而其餘 9 組情境已在 100 次下驗證。
   *
   * 記於 TUNING.md §5。若要消除此偏離，見該節列出的兩個選項。
   */
  repeats?: number;
};
...
const SCENARIOS: readonly Scenario[] = [
  ...
  {
    // 唯一未跑滿 100 次的情境，理由與成本見上方 Scenario.repeats 的說明與 TUNING.md §5。
    name: '逾時平手（跑滿 7200 幀）',
    repeats: 20,
    battle: {
      seed: 20260817,
