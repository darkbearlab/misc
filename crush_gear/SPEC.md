...
> `dist(throwA, throwB) ≥ 2 × VEHICLE_MAX_RADIUS + 0.005`

外接圓對第四輪的官方規格車過度保守到癱瘓投擲區:車長 196 mm、寬僅 90–136 mm,
外接圓半徑由後武器後角決定達 101 mm,但兩台車尾對尾時實際只需約 50 mm 就不重疊。

**新規則:以雙方 yaw 與車體 XZ 投影多邊形做 2D 分離判定。**

1. **外接圓預篩**:中心距 ≥ `2 × VEHICLE_MAX_RADIUS + 0.005` ⇒ 必然滿足,直接通過。
   實測 88.6% 的投擲對在此就通過,只有 11.4% 需要進入下一步。
2. **逐部件 SAT**:兩車各部件的 XZ 投影多邊形兩兩配對,要求
   **不重疊且最小間距 ≥ 0.005 m**。

**逐部件而非整車凸包**:官方規格車的 XZ 投影是**十字形**(外殼 90 寬 × 140 長,
輪武器伸到 136 寬但只有 80 長)。整車凸包會把四個凹角填實,面積遠大於實際車體,
等於只是換一個小一點的保守形狀。每個部件本身都是凸的,因此逐部件配對是**精確**的,
6 × 6 = 36 對的成本可以忽略。

**仍然是純幾何**:不建立世界、不做碰撞查詢,「參數驗證先於模擬」的分層不變。
實作於 `src/sim/separation.ts`,yaw 旋轉直接沿用 `types.ts` 的
`quatFromAxisAngle` / `rotateByQuat`,與模擬用的是同一段程式碼。

**刻意忽略 pitch**:pitch 上限 ±0.3 rad 會讓實際投影略為縮短,忽略它得到略大的
多邊形 —— 偏保守,不會漏判重疊。

**SAT 的軸間隙不是距離**:那只是各面法線方向上的間隙,頂點對頂點的情形會低估。
因此距離另外以「頂點對邊」的最小值精確計算,不沿用 SAT 的結果。
